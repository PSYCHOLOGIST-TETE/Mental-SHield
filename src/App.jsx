...
const TESTS = {
  phq9: {
    ...
    description: "ბოლო 2 კვირის განმავლობაში, რამდენად ხშირად შეგაწუხათ შემდეგმა პრობლემები?",
    icon: "🌧",
    color: "#4A6FA5",
    lightColor: "#EBF0F8",
    options: [
    ...
    questions: [
      ...
      "ინტერესის ან სიამოვნების დაქვეითება საქმიანობებისადმი",
      "ძილის პრობლემები (ძილის სიჭარბე ან ნაკლებობა)",
      "დაღლილობა ან ენერგიის დაქვეითება",
      "მადის ცვლილება (გაზრდა ან შემცირება)",
      "საკუთარი თავის მიმართ უარყოფითი შეგრძნება ან დანაშაულის გრძნობა",
      "კონცენტრაციის სირთულეები",
      "აუჩქარებლობა ან სიძაბუნე სხვებისთვის შესამჩნევად",
      "აზრები იმის შესახებ, რომ სჯობს მკვდარი ყოფილიყავი ან თვითდაზიანების შესახებ",
    ],
    scoring: [
      { min: 0, max: 4, label: "მინიმალური", color: "#22c55e", desc: "თქვენი სიმპტომები მინიმალურია და კლინიკური ჩარევა სავარაუდოდ საჭირო არ არის. ეს კარგი ნიშანია! განაგრძეთ საკუთარი თავის მოვლა — ძილის რეჟიმი, ფიზიკური აქტივობა და სოციალური კავშირები მნიშვნელოვანია განწყობის შესანარჩუნებლად. გირჩევთ ტესტი 1 თვეში გაიმეოროთ." },
      { min: 5, max: 9, label: "მსუბუქი", color: "#84cc16", desc: "მსუბუქი დეპრესიული სიმპტომები შეიმჩნევა. ამ ეტაპზე ცხოვრების წესის გადახედვა ძალიან სასარგებლო შეიძლება იყოს: რეგულარული ვარჯიში, სტრუქტურირებული დღის განრიგი, ახლობლებთან კომუნიკაცია. თუ სიმპტომები 2 კვირაზე მეტ ხანს გრძელდება, გირჩევთ ფსიქოლოგს გაესაუბროთ. ეს ნორმალური ნაბიჯია და სისუსტე არ არის." },
      { min: 10, max: 14, label: "ზომიერი", color: "#f59e0b", desc: "ზომიერი დეპრესიული სიმპტომები გამოვლინდა, რაც ყოველდღიურ ცხოვრებაზე შეიძლება გავლენას ახდენდეს. რეკომენდებულია კვალიფიციურ ფსიქოლოგთან ან ფსიქიატრთან კონსულტაცია. მკურნალობა — იქნება ეს ფსიქოთერაპია თუ სხვა ჩარევა — ამ სტადიაზე ძალიან ეფექტურია. არ გადადოთ." },
      { min: 15, max: 19, label: "მძიმე", color: "#f97316", desc: "მძიმე სიმპტომები გამოვლინდა. ეს ქულა მიუთითებს, რომ პროფესიული დახმარება საჭიროა. გთხოვთ მიმართოთ ფსიქიატრს ან კლინიკურ ფსიქოლოგს რაც შეიძლება მალე. სამკურნალო ვარიანტები — ფსიქოთერაპია, მედიკამენტები ან ორივეს კომბინაცია — ამ სტადიაზე კარგ შედეგს იძლევა." },
      { min: 20, max: 27, label: "ძალიან მძიმე", color: "#ef4444", desc: "ძალიან მძიმე სიმპტომები გამოვლინდა. გთხოვთ, დაუყოვნებლივ მიმართოთ ფსიქიატრს ან უახლოეს ფსიქიკური ჯანმრთელობის სერვისს. თქვენ არ ხართ მარტო — დახმარება არსებობს და გამოჯანმრთელება შესაძლებელია. საქართველოში შეგიძლიათ დაუკავშირდეთ ნდობის ტელეფონს: 116 006." },
    ],
  },
  gad7: {
    id: "gad7",
    ...
    questions: [
      ...
      "უუნარობა, შეჩერდეს ან გააკონტროლოს წუხილი",
    ...
    scoring: [
      { min: 0, max: 4, label: "მინიმალური", color: "#22c55e", desc: "შფოთვის დონე მინიმალურია. ეს ნიშნავს, რომ ამ მომენტში შფოთვა თქვენს ყოველდღიურ ცხოვრებას მნიშვნელოვნად არ აფერხებს. განაგრძეთ ისეთი ჩვევები, რომლებიც გეხმარება — მედიტაცია, ფიზიკური აქტივობა, ბუნებაში სიარული." },
      { min: 5, max: 9, label: "მსუბუქი", color: "#84cc16", desc: "მსუბუქი შფოთვის სიმპტომები შეიმჩნევა. სცადეთ სუნთქვის ვარჯიშები, mindfulness ან ჟურნალის წარმოება. თუ შფოთვა კონკრეტულ სიტუაციებში იჩენს თავს (სოციალური, სამუშაო), ფსიქოლოგთან ერთი-ორი კონსულტაცია ძალიან სასარგებლო შეიძლება იყოს." },
      { min: 10, max: 14, label: "ზომიერი", color: "#f59e0b", desc: "ზომიერი შფოთვა გამოვლინდა, რაც ყოველდღიურ ფუნქციონირებაზე სავარაუდოდ გავლენას ახდენს. კოგნიტურ-ქცევითი თერაპია (CBT) განსაკუთრებით ეფექტურია შფოთვითი აშლილობების სამკურნალოდ. გირჩევთ ფსიქოლოგს მიმართოთ კონსულტაციისთვის." },
      { min: 15, max: 21, label: "მძიმე", color: "#ef4444", desc: "მძიმე შფოთვის სიმპტომები გამოვლინდა. ეს ქულა მიუთითებს, რომ პროფესიული ჩარევა საჭიროა. ფსიქიატრი ან კლინიკური ფსიქოლოგი დაგეხმარებათ სწორი სამკურნალო გეგმის შედგენაში. შფოთვა კარგად ემორჩილება მკურნალობას — ნუ გადადებთ." },
    ],
  },
  pss10: {
    id: "pss10",
    ...
    questions: [
      ...
      "ვერ გამკლავდით ყველა საქმეს, რაც უნდა გაგეკეთებინა",
    ...
    scoring: [
      { min: 0, max: 13, label: "დაბალი სტრესი", color: "#22c55e", desc: "სტრესის დონე ნორმის ფარგლებშია. ეს ნიშნავს, რომ ამ მომენტში კარგად ართმევთ თავს ყოველდღიურ სტრესულ სიტუაციებს. გააგრძელეთ ის, რასაც აკეთებთ — ბალანსის შენარჩუნება მნიშვნელოვანია. გირჩევთ ტესტი ყოველ თვე გაიმეოროთ." },
      { min: 14, max: 26, label: "საშუალო სტრესი", color: "#f59e0b", desc: "საშუალო სტრესი გამოვლინდა, რაც გავრცელებული მდგომარეობაა თანამედროვე ცხოვრებაში. გირჩევთ ყურადღება მიაქციოთ: ძილის ხარისხს (7-9 სთ), ფიზიკურ აქტივობას, ყოველდღიურ გადასვენებებს სამუშაოდან. თუ სტრესი კონკრეტული წყაროდან მოდის (სამუშაო, ურთიერთობა), ფსიქოლოგთან კონსულტაცია სასარგებლო იქნება." },
      { min: 27, max: 40, label: "მაღალი სტრესი", color: "#ef4444", desc: "მაღალი სტრესი გამოვლინდა, რაც ჯანმრთელობაზე — ფიზიკურ და ფსიქიკურ — გავლენას ახდენს. ქრონიკული სტრესი გულ-სისხლძარღვთა სისტემაზე, იმუნიტეტსა და ძილზე უარყოფით ზეგავლენას ახდენს. რეკომენდებულია სპეციალისტის კონსულტაცია და სტრეს-მენეჯმენტის სტრატეგიების დასწავლა." },
    ],
  },
  who5: {
    id: "who5",
    ...
    questions: [
      "გრძნობდით მხიარულებას და კარგ განწყობას",
      "გრძნობდით სიმშვიდეს და დასვენებას",
      "გრძნობდით თავს აქტიურად და სავსე ენერგიით",
      "გაიღვიძეთ მოდუნებულმა და გამძიმებულმა",
      "ყოველდღიური ცხოვრება სავსე იყო საინტერესო საკითხებით",
    ],
    scoring: [
      { min: 0, max: 35, label: "დაბალი კეთილდღეობა", color: "#ef4444", desc: "კეთილდღეობის დაბალი დონე გამოვლინდა. WHO-5-ის ეს ქულა ხშირად დეპრესიის სკრინინგის საჭიროებაზე მიუთითებს. გირჩევთ PHQ-9 ტესტიც გაიაროთ და შედეგი კვალიფიციურ სპეციალისტს გაუზიაროთ. დახმარების ძიება სიძლიერის ნიშანია." },
      { min: 36, max: 67, label: "საშუალო კეთილდღეობა", color: "#f59e0b", desc: "საშუალო კეთილდღეობა გამოვლინდა — ეს ნიშნავს, რომ ზოგ დღეს კარგად გრძნობთ თავს, ზოგ დღეს — ნაკლებად. გაუმჯობესებისთვის ყურადღება მიაქციეთ: მადლიერების პრაქტიკას, სოციალური კავშირების გაძლიერებას, საკუთარი ღირებულებების შესაბამის საქმიანობებს. პატარა ნაბიჯები დიდ ცვლილებებს იწვევს." },
      { min: 68, max: 100, label: "კარგი კეთილდღეობა", color: "#22c55e", desc: "კეთილდღეობის კარგი დონე გამოვლინდა — ეს შესანიშნავია! ეს ნიშნავს, რომ ამ მომენტში ენერგიული, მშვიდი და ცხოვრებით დაინტერესებული ხართ. განაგრძეთ ის ჩვევები, რომლებიც ამ დონეს გინარჩუნებთ. გახსოვდეთ — კეთილდღეობა დინამიური მდგომარეობაა, ყოველთვიური შემოწმება სასარგებლოა." },
    ],
  },
};
